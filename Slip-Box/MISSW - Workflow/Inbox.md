...
- `Frontmatter` - The first section of everyone (the one that it's between the `---`) and, at some point, the most important element of the note^[Only at the *organising point of view*, is more that clear that the content of the note is the most important thing of any note].
	- Here it lays a set of data that it will be use further down the line that gives **key** information about the note at hand. Each one of those data (the word that stands before the `:`) it's called a `key`, and each one of them has a different purpose:
		...
		- `status` - Here its where things gets a little more interesting, but you will see that its not so different than the previous key:
			...
			- Now, What is this for? This represents the `status` of your note. If there is something that needs to be done, specifically about the note that you are writing on, you may need to write it here.
				- If there is more than one tasks that you want to add to the same note, just add another `,` after the last `""`, write another `""` after the `,`, write the task and repeat the number of times that you need.
- `# $=dv.current().alias` - And, finally, you have this thing after a `# `. A `#` followed by a space means that we have a *heading*.
	...
	- Inside the heading there is a `$=dv.current().alias`. That is a *command*, a **Dataview** *command* (or very tiny script if you will). I'm not gonna dig much into it for now, but I'll explain what it does:
		- `dv.current()` - Is a function that retrieves all the meta-data that the actual file can give you. The "meta-data" refers to all the `key`s that the file have, being these `key`s all the ones that every note have by default and the ones that we define inside the `frontmatter`.
...
- **Using Links** - All the `[[Note Name]]` that we write on the notes are `link`s from one note to another. This is a powerful tool, not only to create paths between notes, but also to ***connnect*** ideas one to another. This is very important in the long run for vault, but will talk more about it on [[Linking your ideas|another time]].
...
- **Using Dataview** - Dataview is a plugin that lets you use all the data store on every note of you vault as if it were a database.
	- This vaults make a huge use of said plugin, but I also recommend using `MOC`s, since the use that I give to this plugins is just to organise tasks and files just for enough time for me to move it to a better place (being that place inside the [[Management]] folder or a better suited `MOC`)

So, this `frontmatter` will help us store the small and precise tasks that we have on certain topics/notes more easily, as well as let us retrieve said tasks from other places (*cough cough* [[Issues]] *cough cough*^[I'll explain that code later on the guide, but you can see what is for by seeing the result on the `Preview View` (if you have filled the `frontmatter` on your `Test Report Note` correctly, you should be able to see your task on [[Issues|said]] note)]).
...
- `Document Notes` - You don't need to create a new `Document Note`, since the only difference between this note and the previous note is the use of the tag `docNote`, instead of `repNote`: ^123252
	- Both `tag`s are used to distinguish one type of notes from the other one

Noe then, Whats the difference? This note serves as *the clean up* from the `Report Notes`. This means that the `Document Note` is the formal version of your `Report Note` that, once is done, will go directly into your [[Slip-box]].
...
- **To improve your writing** - This is something that I still struggle myself (and pretty sure will still struggle for a long time). For a long time, there where some people that said that I couldn't explain the things that I had understood correctly, and that wasn't long ago. At first I didn't thought much about it, but as time goes on, this became more of an issue that I had to start addressing.
	- If you had understood the concepts that I'm writing on this guide, that means that I have improved my writing thanks to doing this exact thing: *Writing the stuff I learn with my own words*.
	- And, if you didn't, that means that I failed in my mission to explain it to you, and for that I'm sorry, I really did tried my best. But this is still worth it, since is just another practice round in my journey. A way for you to see if your writing is good or not (or if it has improve over time), is to read what you have written before "putting it away" on your [[Slip-box]] or read what you already have store inside it (something that you may have to do more times that you thing).
